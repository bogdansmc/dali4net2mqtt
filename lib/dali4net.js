...
const lib = require('./devicelist.js');
...
class Dali4Net {
    ...
    getGetHexByName(name) {
	let idx = daliName.indexOf(name);
	if (idx < 0 || idx >= 144) return null;
	let dec = idx*2+1;
	let hex = '0x'+dec.toString(16).toUpperCase();
	return hex;
    }

    getSetHexByName(name) {
	let idx = daliName.indexOf(name);
	if (idx < 0 || idx >= 144) return null;
	let dec = idx*2;
	let hex = '0x'+dec.toString(16).toUpperCase();
...
/*                if (searchDevice[i].indexOf('e') === 0) {
                    ...
                    for(const id in daliClass) {
                        ...
                        data = await this.sendMessage(this.bus[bus].address, transactionId, this.getGetHexByName(deviceName), queryType.edali, daliSize.eDali25Bit, id);
                        this.adapter.log.debug('Data ' + JSON.stringify(data));
                        this.adapter.log.debug('Message received ' + this.getGetHexByName(deviceName));
                        ...
                        if(exists) {
                            ...
                            await this.createClass (typeName, searchDevice[i], this.getGetHexByName(deviceName), this.bus[bus].address, this.bus[bus].name, daliSize.eDali25Bit, id);
                    ...
                    data = await this.sendMessage(this.bus[bus].address, transactionId, this.getGetHexByName(deviceName), queryDali.queryDeviceType, daliSize.dali16Bit, daliClass[0]);
		    ...
		    if (data[14] == 255) data[14] = 6;
                    ...
                    const exists = this.deviceExists(data, searchDevice[i], typeName);
                        ...
                        if(exists) {
                            ...
                            await this.createClass (typeName, searchDevice[i], this.getGetHexByName(deviceName), this.bus[bus].address, this.bus[bus].name, daliSize.dali16Bit, daliClass[0]);
    ...
    async getInfo(bus, address, query, size, deviceClass){
            ...
            const transactionId = this.transactionIdentifier();
            const data = await this.sendMessage(bus, transactionId, address, query, size, deviceClass);
	...
	if (lampStates == null) return;
		...
		if (device.constructor.name == "Lamp") {
    		        ...
    		        //this.adapter.log.debug('levelodl ' + levelOld);
                        ...
                        } else if (this.counterIndex % 60 == 0) {
                    	    // update HomeAssistant values every 60 seconds
                    	    this.adapter.log.debug("force dali level to HA");
                    	    await this.adapter.onDaliLevelChanged(device);
                        }
                }
...
//        	this.adapter.log.debug('Answer Dali4Net lamps: ' + JSON.stringify(data));
    ...
    async sendLampState(bus, value, name) {
        ...
        const lampId = this.getSetHexByName(name);
...
/*
    sendGroupState(bus, value, name) {
        ...
        const groupId = this.getSetHexByName(name);
    ...
    sendScene(bus, name) {
        ...
        const sceneId = this.getSetHexByName(name);
