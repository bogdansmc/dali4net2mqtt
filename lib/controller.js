...
const Dali4Net = require('./dali4net.js');
...
//                "manufacturer": f"{__author__} <{__email__}>",
        ...
        let conf = JSON.stringify(json_config);
	let options = {};
	...
	this.lamps = [];
        ...
        this.dali4net = new Dali4Net(this, this.config.host, this.config.port, 
            this.config.bus0, this.config.bus1, this.config.bus2, this.config.bus3);
        this.dali4net.connect();

        if(this.config.bus0) {
            let dl0 = await this.dali4net.startSearch(0);
            for (let d in dl1) this.lamps[d] = dl0[d];
        };
        if(this.config.bus1) {
    	    let dl1 = await this.dali4net.startSearch(1);
    	    for (let d in dl1) this.lamps[d] = dl1[d];
        };
        if(this.config.bus2) {
            let dl2 = await this.dali4net.startSearch(2);
            for (let d in dl2) this.lamps[d] = dl2[d];
        };
        if(this.config.bus3) {
            let dl3 = await this.dali4net.startSearch(3);
...
//        if(state /*&& state.ack !== true*/ && this.dali4net) {
